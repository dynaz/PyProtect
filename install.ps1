...
if (-not (Test-Path $PYPROTECT_PATH)) {
    Write-Host "❌ pyprotect.py not found in current directory" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "⚠️  IMPORTANT: Restart your terminal to use 'pyprotect' command" -ForegroundColor Yellow
